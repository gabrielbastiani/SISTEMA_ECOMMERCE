...
// ---- constantes (mesmas do seu arquivo original) ----
...
// ---- MultiSelect (idêntico ao anterior) ----
...
export default function PromotionStep2({
  ...
  // Mantemos a mesma API client — guardado em ref para ser estável entre renders
  ...
  // loading local (skeleton) para opções vindas do backend
  const [loadingOptions, setLoadingOptions] = useState(true)

  // Atualiza lógicas quando muda o tipo
  useEffect(() => {
  ...
  // Carrega opções do backend — executa apenas 1 vez (dependências estáveis)
  useEffect(() => {
    let mounted = true
    async function load() {
      ...
      try {
        ...
        console.error('Erro ao carregar opções do PromotionStep2:', err?.message ?? err)
        // mostrar toast de erro mas não travar o flow — o usuário verá placeholders e pode tentar novamente
        toast.error('Falha ao carregar opções. Verifique o console.')
      } finally {
  ...
  }, []) // <<-- executa só uma vez: removemos dependência instável que causava loop
        ...
        return <>
          ...
          {variantOptions.map(v => (
            <label key={v.value} className="block">
              <input type="checkbox"
                checked={payload.variantIds?.includes(v.value) || false}
                onChange={e => {
  ...
  return (
    <div className="space-y-6">
        ...
        <tbody>
          {data.conditions?.map((c: any, i: any) => (
            <tr key={i} className="border-b text-black">
              ...
              <td className="p-2">
                <button onClick={() => removeCondition(i)} className="text-red-600 hover:underline" disabled={disabled}>
                  Remover
                </button>
              </td>
            </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
        <div>
          <label className="block mb-1 font-medium">Condição*</label>
          <Tooltip content="Selecione a condição para a aplicação à Promoção. (Preenchimento Obrigatório)." placement="top-start" className="bg-white text-red-500 border border-gray-200 p-2">
        ...
        <div>
          <label className="block mb-1 font-medium">Lógica*</label>
          <Tooltip content="Selecione a lógica para a aplicar à Promoção. (Preenchimento Obrigatório)." placement="top-start" className="bg-white text-red-500 border border-gray-200 p-2">
      ...
      <div className="flex justify-between">
        <button onClick={onBack} className="px-4 py-2 rounded bg-gray-200 text-black hover:bg-gray-300" disabled={disabled}>
          Voltar
        </button>
        <button onClick={saveCondition} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700" disabled={disabled || loadingOptions}>
          Salvar Condição
        </button>
        <button onClick={onNext} className="px-4 py-2 bg-orange-500 text-white rounded hover:bg-orange-600" disabled={disabled}>
          Próximo
        </button>
      </div>
    </div>
